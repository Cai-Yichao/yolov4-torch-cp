...
def expanding_roi(image):
    roi = cv2.cvtColor(np.asarray(image), cv2.COLOR_RGB2BGR)
    side = roi.shape[0] if roi.shape[0] > roi.shape[1] else roi.shape[1]
    new_image = np.zeros((side, side, 3), np.uint8)
    new_image[:roi.shape[0]-1, :roi.shape[1]-1] = roi[:roi.shape[0]-1, :roi.shape[1]-1]
    result = Image.fromarray(cv2.cvtColor(new_image, cv2.COLOR_BGR2RGB))
    return result


def draw_boxes(image, box, conf, font_path):
...
def post_nms(boxes, conf):
    drop_index = []
    for i in range(len(boxes)):
        x, y, w, h = boxes[i]
        cf = conf[i]
        if w < 0.25 * h or h < 0.25 * w:
            drop_index.append(i)  # drop the box
            continue
        for j in range(len(boxes)):
            ...
            xx, yy, ww, hh = boxes[j]
            cf_e = conf[j]
            if x < (xx + ww) and y < (yy + hh) and (x + w) > xx and (y + h) > yy and cf_e > cf:
                drop_index.append(i)        # drop the box
                break
    if len(drop_index) is 0:
        return boxes, conf
    else:
        ret_boxes = []
        ret_conf = []
        ...
        for i, box in enumerate(boxes):
            ...
            ret_conf.append(conf[i])
        return ret_boxes, ret_conf
